...
*   **Web 管理界面**: 基于 Bootstrap 5 的现代化响应式界面，支持卡片式设备展示。
*   **智能广播 (Smart Broadcast)**: 
    *   支持指定广播 IP。
    *   **新功能**: 如果留空广播 IP，系统将自动遍历所有 IPv4 网络接口发送 Magic Packet，确保在多网卡环境下也能成功唤醒。
*   **日志系统**: 
    *   详细记录唤醒操作、设备增删改及系统错误。
    *   支持按设备筛选查看实时日志。
    *   自动日志轮转与清理（默认保留 3 天）。
*   **设备管理**: 轻松添加、编辑和删除需要唤醒的设备。
...
# 构建 Linux 版本
$env:GOOS='linux'; $env:GOARCH='amd64'; go build -o build/wol-linux
```
...
# 构建 Windows 版本
GOOS=windows GOARCH=amd64 go build -ldflags "-H=windowsgui" -o build/wol-windows.exe
